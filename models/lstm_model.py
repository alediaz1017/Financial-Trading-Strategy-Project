...
# Merge on "County" (inner join keeps only counties present in both files)
...
# We assume your CSV now contains 9 counties and six years of historical prices, for example:
...
for county in df_prices_only.index:
    ...
    # Only form samples if there are at least (input_length + output_length) years
    if len(series) < (input_length + output_length):
        continue
    # Slide a window along the series:
...
# Flatten all values so the scaler learns one global min and max.
...
# Transform the splits by flattening, scaling, and reshaping back.
...
# Reshape X for LSTM: (samples, input_length, features) where features = 1.
...
# 6. Define the LSTM Model for Multi-step Forecasting
...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        # x shape: (batch, input_length, features)
        out, _ = self.lstm(x)
...
hidden_size = 64
output_size = output_length  # predicting 3 values (next 3 years)
model = LSTMModel(input_size, hidden_size, output_size)
criterion = nn.MSELoss()
optimizer = optim.Adam(model.parameters(), lr=0.001)


# 7. Train the LSTM Model and Compute MAPE
...
for epoch in range(num_epochs):
    ...
    # Compute training MAPE on inverse-transformed data
    with torch.no_grad():
        outputs_np = outputs.detach().numpy()
    # Reshape outputs to 2D for inverse_transform then reshape back to original shape
...
# For future prediction, we use the most recent three years (e.g., 2022-2024) as input.
# Ensure these columns exist in your CSV.
future_input_years = ['2022', '2023', '2024']
predictions = {}
for county in df_prices_only.index:
    # Extract the values for future_input_years for the county.
    series = df_prices_only.loc[county][future_input_years].values.astype(float)
    # Only proceed if there are exactly 3 values.
...
df_future_prices_lstm = pd.DataFrame({
    "County": list(predictions.keys()),
    "Predicted Housing Prices (LSTM Future)": list(predictions.values())
})

df_future_prices_lstm.to_csv("future_housing_prices_lstm.csv", index=False)
print("Future LSTM housing prices saved to future_housing_prices_lstm.csv")
print(df_future_prices_lstm)
